...
class LogoTraceLoader extends StatefulWidget {
  ...
  /// While true, the trace loops. When false (or [isComplete] true), resolves to fill.
  final bool loading;

  /// Force completion regardless of [loading].
  final bool isComplete;

  /// Square dimension of the loader.
  final double size;

  /// Stroke width for the animated trace.
  final double strokeWidth;

  /// Duration of one trace loop cycle.
  final double loopDurationSeconds;

  /// Duration of the fill fade-in.
  final double fillFadeSeconds;

  /// Color for stroke and fill. Defaults to [Theme]'s primary color.
  final Color? color;

  /// Called once after the filled logo is fully visible.
...
class _LogoTraceLoaderState extends State<LogoTraceLoader>
  ...
  // Logo SVG data — traced from the Azaman logo PNG.
  static const String _viewBox = "20 20 569 553";

  // Single continuous trace path (3 shapes connected).
  ...
  void initState() {
    super.initState();
    _tracePath = parseSvgPathData(_tracePathData);
    _fillPaths = _fillPathData.map((d) => parseSvgPathData(d)).toList();

    // Measure the total path length for dash calculations.
    final metrics = _tracePath.computeMetrics().toList();
    _pathLength = metrics.fold(0.0, (sum, m) => sum + m.length);

    _loopController = AnimationController(
      vsync: this,
      duration: Duration(milliseconds: (widget.loopDurationSeconds * 1000).round()),
    );

    _fillController = AnimationController(
      vsync: this,
      duration: Duration(milliseconds: (widget.fillFadeSeconds * 1000).round()),
    );
  ...
  void _checkPhaseTransition() {
    ...
    if (shouldComplete && _phase == _Phase.loop) {
      ...
      // After brief outline close, start fading fill.
...
class _LogoTracePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 1. Faint guide path (always visible).
    ...
    if (phase == _Phase.loop) {
      // 2. Animated trace segment — short stroke traveling along the path.
    ...
    // 4. Filled logo (fading in or done).
  ...
  void _drawAnimatedTrace(Canvas canvas, Paint paint) {
    // The trace segment: 16% of total path length, traveling around.
    final segmentLength = pathLength * 0.16;
    final offset = loopProgress * pathLength;

    // Draw the segment using PathMetrics — handles multi-subpath paths.
    ...
    for (final metric in metrics) {
      ...
      // Check two wrap-around windows for the segment.
      for (int i = 0; i < 2; i++) {
