...
///
/// The ordering callback remains the source of truth; this surface only
/// visualizes the already-accepted cart mutation. Reduced-motion users receive
/// the same semantic confirmation without the flying paper animation.
...
class _PaperRipPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final startRect = Rect.fromCenter(
      ...
      width: sheetWidth * reveal,
      height: sheetHeight,
    );
    if (reveal > 0) {
      _paintSheet(canvas, startRect, Colors.white, 1.0);
    }
    ...
    final seam = Paint()
      ..color = const Color(0xFFD9C7A5).withValues(alpha: fade)
      ..strokeWidth = 1.2;
    for (var i = 0; i < 8; i++) {
  ...
  void _paintSheet(Canvas canvas, Rect rect, Color color, double opacity) {
    final shadow = Paint()
      ..color = const Color(0x30000000).withValues(alpha: opacity)
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 10);
    canvas.drawRRect(
      RRect.fromRectAndRadius(rect.shift(const Offset(0, 5)), const Radius.circular(12)),
      shadow,
    );
  ...
  bool shouldRepaint(covariant _PaperRipPainter oldDelegate) =>
      oldDelegate.progress != progress || oldDelegate.textColor != textColor;
}
