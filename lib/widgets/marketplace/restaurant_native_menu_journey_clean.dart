...
class _RestaurantNativeMenuJourneyCleanState extends State<RestaurantNativeMenuJourneyClean> {
  ...
  void didUpdateWidget(covariant RestaurantNativeMenuJourneyClean oldWidget) {
    ...
    if (!identical(oldWidget.sections, widget.sections) || !identical(oldWidget.uncategorisedProducts, widget.uncategorisedProducts)) _pages = _buildPages();
  }
  ...
  List<_MenuPage> _buildPages() {
    ...
    for (final section in widget.sections) addSection(section.name, section.description, section.products);
  ...
  Widget build(BuildContext context) {
    if (_pages.isEmpty) return Center(child: Text('Menu not yet available', style: TextStyle(color: widget.colors.textSecondary)));
          ...
          Padding(
            ...
            child: Align(
              ...
              child: DecoratedBox(
                ...
                child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 7), child: Text(widget.dineInContext!, style: const TextStyle(color: Colors.white, fontSize: 11.5, fontWeight: FontWeight.w700))),
              ),
            ),
          ),
        Expanded(
          child: Padding(
            ...
            child: FlipBookFrame(
              child: FlipBook(
                ...
                idleHint: true,
                material: const BookMaterial(paper: Color(0xFFFDF6E3), castShadow: Color(0x441A1206), edge: Color(0xFFD8C6A2)),
  ...
  Widget _cover() {
    final hero = <BusinessProduct>[...widget.sections.expand((section) => section.products), ...widget.uncategorisedProducts].where((product) => product.primaryImage != null).firstOrNull;
    return _paperSurface(Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      if (hero?.primaryImage != null)
        ClipRRect(borderRadius: BorderRadius.circular(18), child: AzamanNetworkImage(imageUrl: hero!.primaryImage!, width: 120, height: 120, fit: BoxFit.cover))
      else
  ...
  Widget _menuPage(_MenuPage page, int index) => _paperSurface(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(page.title, style: const TextStyle(color: Color(0xFF2D2416), fontSize: 16, fontWeight: FontWeight.w800)),
        if (page.description != null && page.description!.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 4), child: Text(page.description!, style: const TextStyle(color: Color(0xFF8B7A5A), fontSize: 10.5))),
        ...
        child: Padding(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (product.description != null && product.description!.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 2), child: Text(product.description!, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0xFF8B7A5A), fontSize: 10.5))),
          ...
          child: Center(child: TweenAnimationBuilder<double>(
            ...
            child: GestureDetector(
              ...
              child: ConstrainedBox(
                ...
                child: Material(borderRadius: BorderRadius.circular(22), clipBehavior: Clip.antiAlias, color: widget.colors.card, child: _detailBody(product, dish)),
              ),
            ),
          )),
        ),
  ...
  Widget _detailBody(BusinessProduct product, RestaurantDish dish) {
    ...
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Flexible(child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(20, 17, 20, 18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (widget.dineInContext != null && widget.dineInContext!.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 9), child: Row(children: [Icon(Icons.table_restaurant_outlined, size: 15, color: widget.colors.textTertiary), const SizedBox(width: 5), Expanded(child: Text(widget.dineInContext!, style: TextStyle(color: widget.colors.textTertiary, fontSize: 11, fontWeight: FontWeight.w600)))])),
        ...
        ...group.options.map((option) {
          ...
          return CheckboxListTile(
            ...
            onChanged: (value) => setState(() {
              if (value == true) {
                if (!multi) selected..clear()..add(option.id);
  ...
  double _unitPrice(RestaurantDish dish) {
    ...
    for (final variant in dish.variants) if (variant.name == _size) total += variant.priceDelta;
    for (final group in dish.optionGroups) {
      final chosen = _options[group.id] ?? const <String>{};
      for (final option in group.options) if (chosen.contains(option.id)) total += option.priceDelta;
    }
    return total;
  }
  ...
  bool _canAdd(BusinessProduct product, RestaurantDish dish) {
    ...
    for (final group in dish.optionGroups) if (group.required && (_options[group.id]?.isEmpty ?? true)) return false;
    return true;
  }

  Widget _quantityControl() => Container(
  ...
  Widget _commitFeedback() => Positioned.fill(child: IgnorePointer(child: Align(alignment: Alignment.bottomCenter, child: TweenAnimationBuilder<double>(
        tween: Tween(begin: 0.7, end: 1),
        duration: MotionTokens.accessibleDuration(context, MotionTokens.spatial),
        curve: MotionTokens.enter,
        builder: (_, value, __) => Transform.translate(offset: Offset(0, 70 * (1 - value)), child: Opacity(opacity: value, child: Container(
          margin: const EdgeInsets.only(bottom: 18),
          padding: const EdgeInsets.symmetric(horizontal: 15, vertical: 11),
          decoration: BoxDecoration(color: const Color(0xFFFDF6E3), border: Border.all(color: const Color(0xFFD8C6A2)), boxShadow: const [BoxShadow(color: Color(0x44000000), blurRadius: 14, offset: Offset(0, 6))]),
          child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.restaurant_menu_rounded, size: 18, color: Color(0xFF2D2416)), SizedBox(width: 8), Text('Dish added from the menu', style: TextStyle(color: Color(0xFF2D2416), fontSize: 12.5, fontWeight: FontWeight.w800))],
        ))),
      ))));
  ...
  Widget _navButton(IconData icon, String label, bool enabled, VoidCallback onTap) => Semantics(button: true, enabled: enabled, label: label, child: GestureDetector(
        behavior: HitTestBehavior.opaque,
        onTap: enabled ? onTap : null,
        child: AnimatedOpacity(duration: MotionTokens.accessibleDuration(context, MotionTokens.control), opacity: enabled ? 1 : 0.3, child: Container(
          width: 34,
          height: 34,
