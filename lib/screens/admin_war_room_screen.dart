...
class _AdminWarRoomScreenState extends ConsumerState<AdminWarRoomScreen> with SingleTickerProviderStateMixin {
      ...
      child: ListView(
        ...
        children: [
          ...
          if (_allLiveTrades.isEmpty)
            const AzamanEmptyState(icon: Icons.wifi_protected_setup, title: "No Active Trades", subtitle: "No trades are currently live on the network.", compact: true)
          else
      ...
      child: ListView(
        ...
        children: [
          if (_disputes.isEmpty)
            const AzamanEmptyState(icon: Icons.shield, title: "Platform is Secure", subtitle: "No active disputes.", compact: true)
          else
      ...
      child: ListView(
        ...
        children: [
          if (_kycApplications.isEmpty)
            const AzamanEmptyState(icon: Icons.verified_user, title: "No Pending KYC", subtitle: "No pending KYC applications.", compact: true)
          else
      ...
      child: ListView(
        ...
        children: [
          if (_vendorApplications.isEmpty)
            const AzamanEmptyState(icon: Icons.storefront, title: "No Pending Applications", subtitle: "No vendor applications awaiting review.", compact: true)
          else
          ...
          title: Row(
            children: [
              Container(
                ...
                child: Icon(Icons.storefront, color: colors.accent, size: 20),
              ),
          ...
          title: Row(
            children: [
              Container(
                ...
                child: Icon(Icons.person_search, color: colors.warning, size: 20),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                child: Image.network(fullUrl, fit: BoxFit.contain,
                  errorBuilder: (_, __, ___) => Container(
                    ...
                    child: Column(mainAxisSize: MainAxisSize.min, children: [
                      Icon(Icons.broken_image, color: colors.danger, size: 48),
                      const SizedBox(height: 12),
      ...
      child: Container(
        ...
        child: imageUrl != null
                ...
                child: Stack(
                  ...
                  children: [
                    Image.network(
                      ...
                      fit: BoxFit.cover,
                      errorBuilder: (_, __, ___) => Center(child: Icon(Icons.broken_image, color: colors.textTertiary)),
                    ),
                    Positioned(
            ...
            : Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Icon(Icons.image_not_supported_outlined, color: colors.textTertiary, size: 24),
                  const SizedBox(height: 4),
  ...
  Widget _buildStatsGrid(AzamanColors colors) {
    return Row(
      children: [
        Expanded(child: _statCard("Total Users", _stats['totalUsers']?.toString() ?? "0", Icons.people, colors.accent, colors)),
        const SizedBox(width: 10),
        Expanded(child: _statCard("Vol (GHS)", "${_stats['totalFiatVolume'] ?? 0}", Icons.bar_chart, colors.warning, colors)),
        const SizedBox(width: 10),
        Expanded(child: _statCard("Profit", "${_stats['totalAdminProfit'] ?? 0}", Icons.account_balance, colors.success, colors)),
      ],
    );
  }
      ...
      child: Theme(
        ...
        child: ExpansionTile(
          ...
          subtitle: Padding(
            ...
            child: Row(
              children: [
                Icon(Icons.circle, size: 8, color: _getStatusColor(status, colors)),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      _actionButton(Icons.receipt_long, "Receipt", () => _viewProofDialog(trade['proofUrl']), colors),
                      _actionButton(Icons.chat, "Intercept", () => _openChatInterceptor(trade), colors),
                    ],
                  ),
  ...
  void _showGodModeConfirm(BuildContext context, AzamanColors colors, {required bool isBuyerWin}) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            Icon(Icons.warning_amber_rounded, color: isBuyerWin ? colors.success : colors.danger, size: 24),
            const SizedBox(width: 10),
            Expanded(
              child: Text(
      ...
      body: Column(
        children: [
          Container(
            ...
            child: Row(
              children: [
                Icon(Icons.shield_moon, color: colors.danger, size: 16),
                const SizedBox(width: 10),
                Expanded(
                  child: Text(
