# Azaman — Style Reference
> Sticker-covered notebook on cream paper — a vibe of irreverent, minimalist ui vibe fintech that wears its young, orderly aesthetic proudly.

**Theme:** light

Azaman runs a sticker-bombed Gen-Z finance language: warm light canvas,. The signature move is a heavy black hairline (1px) that traces every surface — cards, phone frames, buttons, illustrations — giving the impression of printed stickers peeled onto a page. Radii are slightly rounded (12px for controls, 28–34px for hero cards) to maintain the soft feel without being oversized, while the palette stays disciplined: one neutral canvas, one neutral surface, and three or four muted brand colors used as full-bleed card fills rather than thin accents. Headlines shout at 85–164px with negative tracking; body text retreats to 14–18px Inter. Everything reads as fast, soft, and deliberately unpolished.
...
| Muted Rose | `#d9a5b3` | `--color-muted-rose` | Muted card fills, sticker backgrounds, hero callouts — a soft accent voice |
| Dusty Cyan | `#98b4c5` | `--color-dusty-cyan` | Muted card fills and feature panel backgrounds — secondary muted voice |
| Sage Lime | `#b7c29d` | `--color-sage-lime` | Muted green action color for filled buttons, selected navigation states |
...
White (#ffffff) pill, 12px radius, 1px black border, fixed at the top center of the viewport. Contains the wordmark left-aligned, social icons centered, and a black-bordered 'Get the App' ghost button on the right with a small black square icon. Padding roughly 10px vertical, 20px horizontal.

### Ghost Navigation Button (Get the App)
**Role:** Secondary header action

White background, 1px black border, 12px radius, Inter 14px weight 500, black text, padding 5–10px. Left-side 20px black square icon.
...
### Feature Card (Muted Sticker)

Full-bleed muted fill (Muted Rose #d9a5b3, Dusty Cyan #98b4c5, or Sticker Khaki #e7e3bf), 1px black border, 12px radius, 24px padding. Contains a tilted phone-mockup illustration and a heading. Cards sit on a 34px column gap.

### Sticker Card (Khaki)
**Role:** Neutral feature card

Sticker Khaki (#e7e3bf) fill, 1px black border, 12px radius, 24px padding, no shadow. Used when a card needs visual weight without screaming a brand color.

### Dark Panel Card
**Role:** Inverted surface for bank/account listings

Surface Graphite (#3a3a3a) or Hardware Ink (#000100) fill, white (#ffffff) text, 1px black border, 12–34px radius. Hosts list rows with flag icons, country names, and outlined 'Open' buttons.

### Sage Lime Pill Button
**Role:** Primary action in lists or CTAs

Sage Lime (#b7c29d) background, 1px black border, 12px radius, Inter 14px weight 700, black text, padding 5–10px. The only filled color CTA in the system.

### Outlined Open Button
**Role:** Row-level action inside account lists

White (#ffffff) background, 1px black border, 12px radius, Inter 14px weight 500 black text, padding 5–10px.

### Bank Account Row
**Role:** List item inside Dark Panel Card

White interior surface inside a dark panel, 1px black border, 8–12px radius. Left: 24px square flag icon. Center: country name (Inter 18px weight 700) over muted helper text (Inter 14px weight 400 in #6e6e6e). Right: Outlined Open Button.

### Phone  Frame
**Role:** Product preview

Hardware Ink (#000100) or Surface Graphite (#3a3a3a) body, 1px black border, 32px outer radius with 8–12px offset duplicate frame creating the peeled-sticker effect. Interior shows the app screen on a white card.

### Tilted Sticker Illustration
**Role:** Decorative motif across cards

Muted fills (#d9a5b3, #98b4c5, #b7c29d, #e7e3bf) with 1px black borders, rotated -8° to +12°, sitting on top of card surfaces as if physically pasted on. Often contains bold short copy or iconography.
...
- Reach for the muted brand colors (#d9a5b3, #98b4c5, #b7c29d, #e7e3bf) as full card or panel fills, never as thin text accents
- Set card and button radius to 12px; escalate to 28–34px only for hero-scale panels and phone frames
- Keep the page canvas at #f2f2f2 and default card surface at #ffffff; let the muted stickers do the soft work on top
...
- Do not use the muted brand colors for thin underlines, icon strokes, or 1px borders — they need fill to read as stickers
...
- Do not set radius to 0 or to 9999px; the system lives in the 1px / 12px / 28–34px range
- Do not use the brand colors as text color on white backgrounds — black text on muted fills, white text only on #000100/#3a3a3a panels
...
- accent sticker fills: #d9a5b3 (Muted Rose), #98b4c5 (Dusty Cyan), #b7c29d (Sage Lime), #e7e3bf (Sticker Khaki)
...
3. **Three-Up Feature Card Row**: three cards on a 24px column gap, each 16px radius with 1px #000000 border. Card 1 fill #e7e3bf, Card 2 fill #d9a5b3, Card 3 fill #98b4c5. Each card has 24px padding, a tilted phone mockup inside, and below the row a two-line caption in Inter 18/500 #000000.
...
- **Revolut** — Comparable fintech information architecture (multi-currency account lists inside dark panels), though UGLYCASH swaps Revolut's purple for a muted rose/cyan/lime palette and a brutalist border language
- **Gumroad** — Shares the irreverent Gen-Z tone, sticker-style illustration treatment, and a cream-and-black canvas that lets muted accent colors do the work
...
:root {
  ...
  --color-muted-rose: #d9a5b3;
  --color-dusty-cyan: #98b4c5;
  --color-sage-lime: #b7c29d;
  ...
  --radius-buttons: 12px;
  --radius-hairline: 1px;
  --radius-herocards: 28px;
  --radius-phoneframes: 32px;
  --radius-featurepanels: 34px;
